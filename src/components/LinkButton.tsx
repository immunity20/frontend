...
export const getLinkPxHeight = (height: keyof typeof linkHeights = 'default') => {
  return linkHeights[height];
};

export type LinkButtonProps = {
  fill?: 'outline';
  shadowless?: boolean;
  shape?: 'block' | 'circle' | 'square';
  size?: keyof typeof linkHeights;
  value: string;
  variant?: UIVariant;
};
...
export const LinkButton = styled.a<LinkButtonProps>`
  ...
  ${(p) => `
    height: ${getLinkPxHeight(p.size)}px;
  `};
  ...
  ${(p) =>
    ...
    `
      ...
      width: ${getLinkPxHeight(p.size)}px;
  `};
  ${(p) =>
    p.shape === 'circle' &&
    `
